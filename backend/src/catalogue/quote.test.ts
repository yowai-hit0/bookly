...
import { type Basket, type Quote, QuoteInputError, quoteBasket } from './quote.js';

/**
 * `quoteBasket()`, the API's copy and the authority (plan.md Task 11;
 * data-model_v2.md §6.2, §9.5). The fixture is the same file the frontend suite
 * reads, so a divergence between the two copies fails both projects.
 *
 * Past the fixture, a seeded generator prices thousands of baskets and checks
 * each against an exact BigInt computation that shares no code or formula with
 * the implementation: quotient and remainder, rounded up when the remainder is
...
/** The route's add-on ceiling, plus the package. */
const MAX_PRICES = 51;

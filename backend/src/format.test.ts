...
/** Host zones either side of Kigali, as far apart as the world goes. */
const HOST_ZONES = ['America/New_York', 'Pacific/Pago_Pago', 'Pacific/Kiritimati', 'Asia/Tokyo', 'UTC'];

/** Runs `check` with the process's host timezone set to `zone`, then restores it. */
function withHostTimeZone(zone: string, check: () => void): void {
  const original = process.env.TZ;
  process.env.TZ = zone;
  try {
    check();
...
describe('formatTime', () => {
  ...
  it.each<[string, number]>([
    ['America/New_York', 3],
    ['Pacific/Pago_Pago', 20],
    ['Pacific/Kiritimati', 21],
    ['Asia/Tokyo', 16],
    ['UTC', 7],
  ])('renders the same Kigali clock time with the host in %s', (zone, localHour) => {
    withHostTimeZone(zone, () => {
      // The emulation is real: the host's own clock reads another hour.
      expect(new Date('2026-10-07T07:00:00Z').getHours()).toBe(localHour);
      expect(formatTime('2026-10-07T07:00:00Z')).toBe('09:00');
      expect(formatTime('2026-10-07T22:00:00Z')).toBe('00:00');
    });
  });
...
describe('formatDate', () => {
  ...
  it.each(HOST_ZONES)('never shifts the weekday or the day with the host in %s', (zone) => {
    withHostTimeZone(zone, () => {
      expect(formatDate('2026-10-07')).toBe('Wednesday, 7 October 2026');
      expect(formatDate('2026-10-31')).toBe('Saturday, 31 October 2026');
      expect(formatDate('2027-01-01')).toBe('Friday, 1 January 2027');
    });
  });
...
describe('formatMonth', () => {
  ...
  it.each(HOST_ZONES)('never shifts to a neighbouring month with the host in %s', (zone) => {
    withHostTimeZone(zone, () => {

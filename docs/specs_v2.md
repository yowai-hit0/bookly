...
| **Revision** | 2.2 — reverted frontend and backend to two separately-deployed apps on separate domains (was one deployable in 2.1). CORS allowlist and CSRF token flow reinstated in §7; **R-7 reopened** (§8.2), not solved. 2.1 — audit fixes. §5 gutted in favour of `data-model_v2.md`; seven cross-reference and consistency errors corrected; TOTP, the invite flow, scheduled purges and locale routing removed; R-7 closed (superseded by 2.2). |

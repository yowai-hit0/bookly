...
- [ ] `DESIGN.md` and its sidecar were written after Section 1 and hand-updated after Section 2 (2026-09-20). Re-run `/impeccable document` at the Section 4 milestone and in Phase 4 so they describe what has shipped by then.
...
- [ ] **The `outline` Button variant is nearly invisible on the page.** It draws a Hairline Blue edge (`border-border`) on a page-coloured fill, about 1.1:1, so every outline button (Retry, cancel, filters, and in Section 2 the time and month buttons) reads as floating text. Section 2 overrides it locally with `border-input bg-card`. Decide in Phase 4 whether to change the variant itself, which is one class in `button.tsx` but changes every outline button in the admin too.

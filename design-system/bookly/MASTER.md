...
- **Selectable card** (package and add-on rows now; `PaymentFields` in Section 3): `bg-card border shadow-sm`, hover `not-has-checked:hover:bg-muted/60`; selected is `has-checked:border-primary has-checked:bg-primary/5 has-checked:ring-1 has-checked:ring-primary`, a 2px green edge and a 5% tint on top of the native radio or checkbox mark; keyboard focus is `has-focus-visible:border-ring` plus `ring-3 ring-ring/50`, which overrides the checked ring (verified in the browser).
...
- **Slot picker:** bookable days are `bg-muted font-semibold` (hover 8% darker), unavailable days are `disabled` with 50% text and no surface, and the selected day is `bg-primary`. Day cells are 40px (44px on touch) and take the same focus edge as controls; time buttons are `h-11` (44px everywhere, as the page file says).
